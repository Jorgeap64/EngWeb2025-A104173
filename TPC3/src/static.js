...
export function staticResource(request){
    return /\/w3.css$/.test(request.url) || 
            /\/favicon.png$/.test(request.url) ||
            /\/student.png$/.test(request.url)
}

export function serveStaticResource(req, res){
    var partes = req.url.split('/');
    var file = partes[partes.length -1 ];
    
    let filePath;
    if (file === 'w3.css') {
        filePath = `TPC3/public/styles/`;
    } else {
        filePath = `TPC3/public/static/`;
    }

    fs.readFile(filePath + file, (erro, dados)=>{
        if(erro){
